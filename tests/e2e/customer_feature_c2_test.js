...
Scenario("C2-TC02 customer can open menu page with cart and restaurant query", async ({ I }) => {
...
Scenario("C2-TC06 customer can open order lookup history page", async ({ I }) => {
  I.amOnPage("/orders/lookup/history");
  I.seeInCurrentUrl("/orders/lookup");
  I.seeElement("body");
});

Scenario("C2-TC07 customer can open restaurant detail by id route", async ({ I }) => {
  I.amOnPage("/restaurant/1");
  I.seeInCurrentUrl("/restaurant/1");
  I.seeElement("body");
});

Scenario("C2-TC08 customer can open restaurant detail by slug route", async ({ I }) => {
  I.amOnPage("/restaurants/demo-restaurant");
  I.seeInCurrentUrl("/restaurants/demo-restaurant");
  I.seeElement("body");
});

Scenario("C2-TC09 customer can open order lookup with phone query", async ({ I }) => {
